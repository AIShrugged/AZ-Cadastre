...
| **Profile**  | A Verification Profile: the rules a submission is judged against. Lives in code, not the database (ADR-0002).                                                                                                                                                                   | A user profile, in every other system.                                                                                                                                                                                                    | There are accounts now, and the other one is an **Account** — never a profile, and never a **Member** either, which was this row's own earlier guess (ADR-0029).                                                                                                                                                                                                                                                         |
| **Login**    | Nothing: verification has no word for a person. In **accounts** it is the identifier an account answers to — a plain string, folded to lower case, three to sixty-four characters, unique, and deliberately **not** an email address (the office's own is `cadastre-operator`). | Everywhere else in this repo, "login" is the act: `POST /api/auth/login`, `auth.login()`, signing in. The accounts context's own word for the act is **authentication**.                                                                  | `login` as a noun is the identifier and only the identifier — on `AccountDto`, on the request bodies, and as the column. The act is spelled **sign in** in prose and **authenticate** in the context's code; `/auth/login` keeps its name because it is a published route. Never call the identifier an email, and never name a column `email` for it (ADR-0029).                                                        |
